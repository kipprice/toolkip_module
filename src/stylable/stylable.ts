import { IStandardStyles, IThemeColors, ICustomFonts, TypedClassDefinition, IFontFaceDefinition } from '../styleHelpers/_interfaces';
import { format } from '../primitiveHelpers/strings';
import { createStyleElement, createFontDefinition, createClass } from '../styleHelpers/styles';
...
export abstract class Stylable extends NamedClass {
    ...
    protected static _sharedCreateStyles(styles: IStandardStyles, elems: HTMLStyleElement[], forceOverride?: boolean): HTMLStyleElement[] {
        ...
        map(styles, (cssDeclaration: TypedClassDefinition, selector: string) => {
            let tmpElem: HTMLStyleElement = createClass(selector, cssDeclaration, true, forceOverride, true);
            if (!tmpElem) { return; }
