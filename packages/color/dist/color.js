 ...
 *
 */
class _Color extends toolkip_named_class_1._NamedClass {
    //#endregion
    //#region CONSTRUCTOR
    /**
    ...
    */
    ...
    get luminance() {
        if (toolkip_shared_types_1.isNullOrUndefined(this._luminance)) {
            this._calculateLuminance();
        }
     ...
     *
     */
    toHexString(with_alpha) {
        ;
        let out;
        out = "#";
        out += toolkip_primitive_helpers_1.fullHexString(this._red, 2);
        out += toolkip_primitive_helpers_1.fullHexString(this._green, 2);
        out += toolkip_primitive_helpers_1.fullHexString(this._blue, 2);
        if (with_alpha) {
            out += toolkip_primitive_helpers_1.fullHexString(this._alpha, 2);
        }
        return out;
    }
    ;
    ...
    _parseFromHexColor(hex, alpha) {
        ...
        if (toolkip_primitive_helpers_1.charAt(hex, 0) === "#") {
            hex = toolkip_primitive_helpers_1.rest(hex, 1);
        }
        if (hex.length < 6) {
            inc = 1;
        }
     ...
     *
     */
    updateRgbValue(valueType, val) {
        switch (valueType) {
            case _interfaces_1.RGBEnum.RED:
                this.red = val;
                break;
            case _interfaces_1.RGBEnum.GREEN:
                this.green = val;
                break;
            case _interfaces_1.RGBEnum.BLUE:
                this.blue = val;
                break;
            case _interfaces_1.RGBEnum.ALPHA:
                this.alpha = val;
                break;
        }
    }
     ...
     *
     */
    updateHslValue(valueType, val) {
        switch (valueType) {
            case _interfaces_1.HSLPieceEnum.HUE:
                this.hue = val;
                break;
            case _interfaces_1.HSLPieceEnum.SATURATION:
                this.saturation = val;
                break;
            case _interfaces_1.HSLPieceEnum.LIGHTNESS:
                this.lightness = val;
                break;
            case _interfaces_1.HSLPieceEnum.ALPHA:
                this.alpha = val;
                break;
        }
    }
     ...
     *
     */
    getNextColor(firstRotate, withAlpha) {
        ;
        var toCycle = [], idx;
    ...
    rotateHue() {
        ;
        this._hue = this.rotateHslValue(this._hue, _constants_1.HUE_INTERVAL, 360);
        return this._hue;
    }
    ;
    /**
    ...
    rotateSaturation() {
        ;
        return this._saturation = this.rotateHslValue(this._saturation, _constants_1.SATURATION_INTERVAL, 100, _constants_1.SATURATION_LIMITS.max, _constants_1.SATURATION_LIMITS.min);
    }
    ;
    /**
     * rotateLightness
     ...
     */
    rotateLightness() {
        return this._lightness = this.rotateHslValue(this._lightness, _constants_1.LIGHT_INTERVAL, 100, _constants_1.LIGHTNESS_LIMITS.max, _constants_1.LIGHTNESS_LIMITS.min);
    }
    /**
     * rotateHslValue
     *
     ...
     *
     */
    rotateHslValue(startVal, inc, modBy, max, min) {
        ;
        var out;
        // Increment and mod
        out = startVal += inc;
        ...
        if (!max) {
            return toolkip_maths_1.roundToPlace(out, 10);
        }
        if (!min && (min !== 0)) {
            return toolkip_maths_1.roundToPlace(out, 10);
        }
     ...
     *
     */
    compare(other_color, multipliers) {
        ;
        var diffs;
     ...
     *
     */
    averageIn(other_color, no_merge) {
        ;
        let avgs;
 ...
 *
 */
class RGBColor extends _Color {
    /** Creates an RGB Color */
    constructor(redOrRgbString, greenOrAlpha, blue, alpha) {
        super();
 ...
 *
 */
class HSLColor extends _Color {
    /** Creates an HSL Color */
    constructor(hueOrHslString, saturationOrAlpha, lightness, alpha) {
        super();
