...
import { clonePointArray } from '@kipprice/toolkip-objectHelpers/cloning";
import { getAngle, getDistance, getEndPoint } from '@kipprice/toolkip-maths/trig";
...
export class PathElement extends _CanvasElement {
	...
	private _scalePoint (pt: IPoint, center: IPoint, scaleAmt: number) : IPoint {
		let angle: number = getAngle(center, pt);
		let distance: number = getDistance(center, pt);
		let newDistance: number = distance * scaleAmt;
		let newPt: IPoint = getEndPoint(center, angle, newDistance);
		return newPt;
	}
}
