import { createSVGElement, IElemDefinition } from '@toolkip/create-elements';
import { TypedClassDefinition } from '@toolkip/style-helpers';
import { loadFile } from '@toolkip/server';
...
export async function embedSvgImage(src: string, style?: Partial<TypedClassDefinition>): Promise<SVGElement> {
	let elem = createSVGElem({ type: "svg", style: style });
	let fc = await loadFile({ filename: src });
	elem.innerHTML = fc;
	return elem;
}
